...
async function getPopularMovies() {
  ...
  function handleFavorite(elFavoriteImg, movie) {
    if (elFavoriteImg.target.classList.contains('movie-favorited')) {
     ...
     removeMovieLocalStorage(movie.id);
    
    } else  {
        elFavoriteImg.target.classList.add('movie-favorite');
  ...
  function removeMovieLocalStorage(id) {
    const movies = getFavoriteMovies() || [];
 ...
 function renderMovie(movie) {
  ...
  const imgFavorite = document.getElementById(`icon-favorite-movie-${id}`);
  imgFavorite.addEventListener('click', (elFavoriteImg) => handleFavorite(elFavoriteImg, movie));
}